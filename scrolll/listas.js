const megadescricoes = [
    'Uma linha que vai para a direita', 'A cor cinza e a sua importância', 'Como organizar o conteúdo de uma gaveta', 'O impacto das sombras nas paredes de uma sala', 'A evolução do som do silêncio', 'O papel das paredes na decoração', 'Como o vento muda de direção', 'O tamanho de uma maçã em comparação com uma laranja', 'Vendo uma gota de água cair', 'Uma nuvem no céu e nada mais', 'O processo de abrir uma porta lentamente', 'Observando uma folha de papel amassada', 'A profundidade de um prato vazio', 'A história do último pedaço de pão', 'Uma cadeira e a sua relação com o chão', 'Vendo um fio de cabelo cair', 'A durabilidade da tinta branca nas paredes', 'A viagem de um grão de poeira', 'Quando uma porta range', 'O comportamento de um botão no tecido', 'A sequência de respirações de uma pessoa', 'O que acontece quando um relógio bate a meia-noite', 'O mistério de um sapato perdido', 'A posição de uma lâmpada apagada', 'A expectativa de um olhar distante', 'O peso de um pensamento sem forma', 'O que faz uma cadeira quando ninguém está a usá-la', 'A leveza de um suspiro', 'A cor que nunca chega', 'Como a luz se espalha por um quadro branco', 'O som do vento em uma rua vazia', 'O primeiro minuto de um minuto', 'O silêncio entre duas palavras', 'A sensação de um espaço vazio', 'A comparação entre uma linha reta e uma curva', 'A dança lenta de um pedaço de papel no ar', 'A importância de uma sombra em movimento', 'A diferença entre uma folha e uma folha amassada', 'Quando a chuva não cai', 'A posição de uma sombra ao meio-dia', 'A importância de uma pausa silenciosa', 'Vendo uma folha de grama balançar', 'O processo de descer uma escada', 'Uma bolha de ar que não estoura', 'O que acontece quando uma gota de chuva não toca o chão', 'O silêncio de um segundo que não passou', 'A arte de observar um relógio sem ver o tempo passar', 'O enigma de uma janela aberta', 'O que um farol faz quando está apagado', 'Como uma xícara fica quieta na mesa', 'A leveza de uma brisa em um dia quente', 'A importância de um pingo de tinta numa tela em branco',
