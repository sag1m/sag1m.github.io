...
// Função para gerar a cor baseada no progresso (gradiente entre #00ebff, #14ff00, #ff0000)
function gerarCor() {
  // O progresso vai de 0 a 1, mapeando para 3 cores principais (azul, verde, vermelho)
  const progress = progresso * 3; // 3 etapas do gradiente
  ...
  const cores = [
    ...
    { r: 20, g: 255, b: 0 },     // Verde claro (#14ff00)
    { r: 255, g: 0, b: 0 }       // Vermelho (#ff0000)
  ];

  // Interpolação entre as cores
  const c1 = cores[i];
  const c2 = cores[(i + 1) % 3]; // Garante que o índice não ultrapasse o array de cores
...
// Função que atualiza o progresso da animação de forma muito mais lenta
function atualizarProgresso() {
  progresso += 0.0002778 * direcao; // Incrementa ou decrementa o progresso com base na direção (fazendo o ciclo durar 1 minuto)
  if (progresso >= 1 || progresso <= 0) {
    direcao *= -1; // Inverte a direção quando chega a 0 ou 1
  }
  // Aplica a cor a todos os elementos
  aplicarCor();
}

// Inicia a animação (1 minuto = 60000ms, 1 ciclo de cor = 60 segundos)
